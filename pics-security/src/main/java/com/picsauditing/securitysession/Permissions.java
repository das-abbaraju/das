...
import com.picsauditing.securitysession.builder.PermissionsBuilder;
...
public class Permissions implements Serializable {
    ...
    public Permissions loginWithoutVerifyingLanguage(User user) {
        ...
        active = user.getIsActive() == YesNo.Yes;
        ...
        shadowedUserID = (UserService.getShadowedUser(user) != null ? UserService.getShadowedUser(user).getId() : userID);
        shadowedUserName = (UserService.getShadowedUser(user) != null ? UserService.getShadowedUser(user).getName() : username);
	...
	public void setAccountPerms(User user) {
        ...
        if ("Contractor".endsWith(accountType)) {
            showClientSitesLink = AccountService.isCanContractorAddClientSites(((ContractorAccount) user.getAccount()));
        }
		...
		if (isOperatorCorporate()) {
			...
			visibleAuditTypes = AccountService.getVisibleAuditTypes(operator);

			approvesRelationships = !operator.isAutoApproveRelationships()
					|| UserService.hasPermission(user, OpPerms.ViewUnApproved);

			gcFree = AccountService.isGeneralContractorFree(operator);

			if (generalContractor || gcFree) {
				for (OperatorAccount linkedClient : AccountService.getLinkedClientSites(operator)) {
					linkedClients.add(linkedClient.getId());
				}
			}

			if (AccountService.getLinkedGeneralContractorOperatorAccounts(operator).size() > 0) {
				for (OperatorAccount generalContractor : AccountService.getLinkedGeneralContractorOperatorAccounts(operator)) {
		...
		for (com.picsauditing.securitysession.entities.UserAccess ua : UserService.getPermissions(user)) {
	...
	public boolean isCanAddRuleForOperator(OperatorAccount operator) {
		...
		if (operator != null) {
			if (isPicsEmployee() && (operator.getStatus() == AccountStatus.Demo || operator.getStatus() == AccountStatus.Pending)) {
				return true;
			}
