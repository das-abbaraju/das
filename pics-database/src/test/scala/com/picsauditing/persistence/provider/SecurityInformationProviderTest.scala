...
class SecurityInformationProviderTest extends BaseTestSetup {
  ...
  trait SecurityDataTest extends DBTest[SecurityInformationProvider] {
    ...
    val findThis = UserData(None, TEST_ACCOUNT_ID, "xx", "Nobody Special", "email", "phone", "fax", "Yes", LATEST_DATE)
    val testUser2 = UserData(None, TEST_ACCOUNT_ID, "xx", "Still Nobody Special", "email", "phone", "fax", "Yes", EARLIER_DATE)
    val testUser3 = UserData(None, TEST_ACCOUNT_ID, "xx", "Still Nobody Special", "email", "phone", "fax", "Yes", EARLIER_DATE)
    val dontFindThis = UserData(None, SOME_OTHER_ID, "xx", "Still Nobody Special", "email", "phone", "fax", "Yes", IMPOSSIBLE_DATE)
