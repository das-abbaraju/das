...
public class PicsMenu {
	...
	static public MenuComponent getMenu(Permissions permissions) {
		...
		if (permissions.hasPermission(OpPerms.Billing)) {
			...
			subMenu.addChild("Lifetime Membership", "ReportLifetimeMembership.action");
		}

		subMenu = menu.addChild("InsureGuard");
		if(permissions.hasPermission(OpPerms.InsuranceCerts))
			subMenu.addChild("Contractor Policies", "ReportPolicyList.action?filter.auditStatus=Submitted&filter.auditStatus=Resubmitted&filter.auditStatus=Active");
		...
		if (permissions.hasPermission(OpPerms.InsuranceApproval)) {
			subMenu.addChild("Policies Awaiting Decision", "ReportInsuranceApproval.action?filter.caoStatus=Awaiting&filter.auditStatus=Submitted&filter.auditStatus=Active&filter.auditStatus=Resubmitted");
