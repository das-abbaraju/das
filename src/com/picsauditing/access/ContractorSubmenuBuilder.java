...
public final class ContractorSubmenuBuilder {
	...
	// todo: Not good that we're injecting and calling methods on a controller
	// here. This is a interim approach that does not change the existing code.
	// Eventually, we need to extract/rework all the logic in
	// ContractorActionSupport and dependencies that builds the auditmenu.
	protected List<MenuComponent> buildAuditMenu(Permissions permissions, ContractorAccount contractorAccount)
	...
	protected MenuComponent addCompanyMenu(MenuComponent menubar, Permissions permissions,
		...
		companyMenu.addChild(getText(DASHBOARD), "ContractorView.action?id=" + contractorAccount.getId());

		companyMenu.addChild(getText(NOTES), "ContractorNotes.action?id=" + contractorAccount.getId());

		if (!permissions.isOperator() && !permissions.isInsuranceOnlyContractorUser()) {
			companyMenu.addChild(getText(CLIENT_SITES), "ContractorFacilities.action?id=" + contractorAccount.getId());
		}

		if (permissions.isGeneralContractor()) {
			companyMenu.addChild(getText(SUBCONTRACTOR_CLIENT_SITES), "SubcontractorFacilities.action?id="
					+ contractorAccount.getId());
		}

		if (permissions.isAdmin() || (permissions.isContractor() && permissions.hasPermission(OpPerms.ContractorAdmin))) {
			companyMenu.addChild(getText(USERS), "UsersManage.action?account=" + contractorAccount.getId());
		}

		if (!permissions.isContractor() || permissions.hasPermission(OpPerms.ContractorSafety)) {
			companyMenu.addChild(getText(TRADES), "ContractorTrades.action?id=" + contractorAccount.getId());
		}

		if (permissions.isContractor()) {
			companyMenu.addChild(getText(RESOURCES), "Resources.action");
		}
		// todo: Investigate. For some reason the conHeader.jsp is showing these
		// erroneously?
		if (permissions.isAdmin() && permissions.getAccountStatus() != AccountStatus.Demo) {
			companyMenu.addChild(getText(BILLING_DETAIL), "BillingDetail.action?id=" + contractorAccount.getId(),
					"billing_detail");
			companyMenu.addChild(getText(PAYMENT_OPTIONS),
					"ContractorPaymentOptions.action?id=" + contractorAccount.getId());
		}

		if (permissions.isAdmin()) {
			companyMenu.addChild(getText(COMPANY_PROFILE), "ContractorEdit.action?id=" + contractorAccount.getId(),
					"edit_contractor");
		}
	...
	protected MenuComponent addAuditguardMenu(MenuComponent menubar, Permissions permissions,
			ContractorAccount contractorAccount, List<MenuComponent> auditMenu) {

		MenuComponent auditguard = menubar.addChild(getText(AUDITGUARD));

		if (!permissions.isContractor() || permissions.hasPermission(OpPerms.ContractorSafety)) {
			auditguard.addChild(getText(AUDITGUARD_SUMMARY),
					"ContractorDocuments.action?id=" + contractorAccount.getId() + "#auditguard");

			MenuComponent menuComponent = findComponent(AUDITGUARD, MANUAL_AUDIT, auditMenu);
			if (menuComponent != null) {
				auditguard.addChild(expandYearToFourDigits(menuComponent.getName()), "Audit.action?auditID="
						+ menuComponent.getAuditId());
			}
	...
	protected MenuComponent addDocuguardMenu(MenuComponent menubar, Permissions permissions,
			ContractorAccount contractorAccount, List<MenuComponent> auditMenu) {
		...
		if (!permissions.isContractor() || permissions.hasPermission(OpPerms.ContractorSafety)) {
			docuguard
					.addChild(getText(DOCUGUARD_SUMMARY), "ContractorDocuments.action?id=" + contractorAccount.getId());

			MenuComponent menuComponent = findComponent(PQF, PQF, auditMenu);
			if (menuComponent != null) {
				// todo: See if there is an effectiveDate on the audit and
				// append the yyyy string to the name
				docuguard.addChild(getText(PQF_CURRENT_YEAR), "Audit.action?auditID=" + menuComponent.getAuditId());
			}
			// todo: See if there is any way to find and append the yyyy string
			// to the name
			docuguard.addChild(getText(ANNUAL_UPDATE_CURRENT_YEAR), "ContractorDocuments.action?id="
					+ contractorAccount.getId() + "#" + ContractorDocuments.getSafeName(getText("AuditType.11.name")));
		}

		return menubar;
	}

	protected MenuComponent addInsureguardMenu(MenuComponent menubar, Permissions permissions,
			ContractorAccount contractorAccount, List<MenuComponent> auditMenu) {
		...
		if (!permissions.isContractor() || permissions.hasPermission(OpPerms.ContractorInsurance)) {
			...
			insureguard.addChild(getText(INSUREGUARD_SUMMARY), "ConInsureGUARD.action?id=" + contractorAccount.getId()); // todo:
																															// Verify.
																															// The
																															// Summary
																															// and
			...
			if (menuComponent != null) {
				insureguard.addChild(getText(CERTIFICATES_MANAGER),
						"ConInsureGUARD.action?id=" + contractorAccount.getId());
			}

			menuComponent = findComponent(INSUREGUARD, AUTOMOBILE_LIABILITY, auditMenu);
			if (menuComponent != null) {
				insureguard.addChild(expandYearToFourDigits(menuComponent.getName()), "Audit.action?auditID="
						+ menuComponent.getAuditId());
			}
			...
			if (menuComponent != null) {
				insureguard.addChild(expandYearToFourDigits(menuComponent.getName()), "Audit.action?auditID="
						+ menuComponent.getAuditId()); // todo: Remove "/" from
														// the english
														// translation?
			}
	...
	protected MenuComponent addEmployeeguardMenu(MenuComponent menubar, Permissions permissions,
			ContractorAccount contractorAccount) {

		MenuComponent employeeguard = menubar.addChild(getText(EMPLOYEEGUARD));

		if (contractorAccount.isHasEmployeeGUARDTag()
			...
			if (permissions.isAdmin() || permissions.hasPermission(OpPerms.ContractorAdmin)) {
				employeeguard.addChild(getText(EMPLOYEES), "ManageEmployees.action?id=" + contractorAccount.getId());
			}

			if (permissions.isAdmin() || permissions.hasPermission(OpPerms.DefineRoles)) {
				employeeguard.addChild(getText(JOB_ROLES), "ManageJobRoles.action?id=" + contractorAccount.getId());
			}
	...
	private String expandYearToFourDigits(String name) {
		return name.replace("'12", "2012").replace("'13", "2013").replace("'14", "2014").replace("'15", "2015")
				.replace("'16", "2016");
	}

	// todo: Revisit. Fragile. For now, we'll reuse the code that builds
	// auditMenu.
