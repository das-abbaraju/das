    ...
    public List<String> getOpenTasks() {
        if (openTasks == null) {
            ...
            if (contractor.getBalance().compareTo(BigDecimal.ZERO) > 0) {
                for (Invoice invoice : contractor.getInvoices()) {
                    if (invoice.getStatus().isUnpaid()) {
                        ...
                        openTasks
                                .add("You have an <a href=\"InvoiceDetail.action?invoice.id="
                                        + invoice.getId()
                                        + "\">invoice of <b>$"
                                        + invoice.getBalance()
                                        + "</b></a> due " + due);
            ...
            if (!contractor.isPaymentMethodStatusValid()) {
                openTasks
                        .add("Please <a href=\"ContractorPaymentOptions.action?id="
                                + contractor.getId()
            ...
            for (ContractorAudit conAudit : getActiveAudits()) {
                ...
                if (conAudit.getAuditType().getClassType().isPqf()) {
                    ...
                    if (conAudit.getAuditStatus().isPending()
                        ...
                        openTasks.add("Please <a href=\"Audit.action?auditID="
                                + conAudit.getId()
                                + "\">complete and submit your " + auditName
                                + "</a>");
                    } else if (conAudit.getAuditStatus().isActiveSubmitted()
                            && conAudit.isAboutToExpire()) {
                        openTasks.add("Please <a href=\"Audit.action?auditID="
                                + conAudit.getId()
                                + "\">review and re-submit your " + auditName
                                + "</a>");
                    }
                }
                if (conAudit.getAuditType().isAnnualAddendum()
                        && (conAudit.getAuditStatus().isPending() || conAudit
                ...
                if (conAudit.getAuditType().getClassType() == AuditTypeClass.Policy) {
                    ...
                    for (ContractorAuditOperator cAuditOperator : conAudit
                            .getCurrentOperators()) {
                        if (CaoStatus.Pending
                                .equals(cAuditOperator.getStatus())) {
                            AuditOperator ao = cAuditOperator.getOperator()
                                    .getAuditMap().get(
                                            cAuditOperator.getAudit()
                                                    .getAuditType().getId());
                            if (ao.isCanSee() && ao.getMinRiskLevel() > 0) {
                                pendingOperators.add(cAuditOperator
                                        .getOperator().getName());
                            }
                        }
                    }
                    ...
                    for (ContractorAuditOperator cAuditOperator : conAudit
                            .getCurrentOperators()) {
                        if (CaoStatus.Rejected.equals(cAuditOperator
                                .getStatus())) {
                            AuditOperator ao = cAuditOperator.getOperator()
                                    .getAuditMap().get(
                                            cAuditOperator.getAudit()
                            ...
                            if (ao.isCanSee()
                                    ...
                                    && ao.getMinRiskLevel() <= contractor
                                            .getRiskLevel().ordinal()) {
                                String Text = "<a href=\"Audit.action?auditID="
                                        + conAudit.getId()
                                        + "\">Update your "
                                        + conAudit.getAuditType()
                                                .getAuditName()
                                        + " Policy rejected by </a> "
                                        + cAuditOperator.getOperator()
                                                .getName();
                                if (!Strings.isEmpty(cAuditOperator.getNotes()))
                                    Text += " for reason "
                                            + cAuditOperator.getNotes();
                                openTasks.add(Text);
                            }
                        }
                    }
            ...
            if (!contractor.isNaicsValid()) {
                ...
                if (auditCatData != null)
                    openTasks
                            .add("Please <a href=\"AuditCat.action?auditID="
                                    + auditCatData.getAudit().getId()
                                    + "&catDataID="
                                    + auditCatData.getId()
    ...
    public boolean isReminderTask() {
        ...
        if (Calendar.getInstance().get(Calendar.MONTH) == 0)
            if (contractor.getViewedFacilities() == null) {
                ...
                if (!sdf.format(contractor.getViewedFacilities()).equals(
                        sdf.format(new Date()))) {
                    return true;
                }
            }
    ...
    private AuditCatData getAuditCatData(ContractorAccount contractor) {
        for (ContractorAudit contractorAudit : contractor.getAudits()) {
            if (contractorAudit.getAuditType().isPqf()) {
                for (AuditCatData auditCatData : contractorAudit
                        .getCategories()) {
