...
public class CommissionModel {
	...
	private void buildClientSiteServiceLevels(StringBuilder clientSiteServiceLevels, List<CommissionDetail> commissionDetails) {
		...
		for (CommissionDetail commissionDetail : commissionDetails) {
			...
			clientSiteServiceLevels.append("\"").append(commissionDetail.getServiceLevels()).append("\"").append(Strings.NEW_LINE);
	...
	private void buildServiceSummary(StringBuilder serviceSummary, Map<FeeClass, Integer> serviceSummaryMap, Invoice invoice) {
		serviceSummary.append("Service Summary").append(Strings.NEW_LINE);
		serviceSummary.append("Service").append(",")
				.append("Total Sites Using Service").append(",")
				.append("Commission Eligible").append(",")
				.append("Total Amount from Invoice").append(Strings.NEW_LINE);
		...
		for (Map.Entry<FeeClass, Integer> entry : serviceSummaryMap.entrySet()) {
			...
			serviceSummary.append(feeTotals.get(entry.getKey()) == null ? "0" : feeTotals.get(entry.getKey()).doubleValue()).append(",");
	...
	private void buildMathForBreakdown(StringBuilder mathForBreakdown, List<CommissionDetail> commissionDetails, Map<FeeClass, Integer> serviceSummary, List<CommissionAudit> commissionAudits, Invoice invoice) {
		...
		for (CommissionDetail commissionDetail : commissionDetails) {
			...
			mathForBreakdown.append("\"").append(commissionDetail.getAccountRepresentativeName()).append("\"").append(",");
	...
	private void buildMathForBreakdownHeader(StringBuilder mathForBreakdown, Set<FeeClass> services) {
		...
		for (FeeClass feeClass : services) {
			...
			mathForBreakdown.append("Weighted ").append(feeClass.name()).append(" (total amount from invoice / total sites using service)").append(",");
		}
		...
		mathForBreakdown.append("Aggregated Weight (sum of per client site weighted services / Total Commission Eligible Services)").append(",");
