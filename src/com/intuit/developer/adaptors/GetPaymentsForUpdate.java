...
public class GetPaymentsForUpdate extends PaymentAdaptor {
	...
	public String getQbXml(QBSession currentSession) throws Exception {
		...
		if( currentSession.getPossiblePaymentUpdates().size() > 0 ) {
			...
			for( Invoice invoice : currentSession.getPossiblePaymentUpdates() ) {
				currentSession.getCurrentBatch().put(invoice.getQbPaymentListID(), new Integer( invoice.getId() ).toString() );
			}
	...
	public Object parseQbXml(QBSession currentSession, String qbXml)
		...
		Map<String, Map<String, Object>> parsedResponses = parsePaymentQueryResponse(qbXml, currentSession.getCurrentBatch());
