...
public class InvoiceAdaptor extends QBXmlAdaptor {
	...
	/*
	public String deleteTheseInvoices( Map<String, Map<String,Object>> qbIds ) throws Exception {
		Writer writer = makeWriter();	
		...
		for( String thePk : qbIds.keySet() ) {
			...
			deleteRequest.setListDelType("Invoice");
	...
	public String updateTheseInvoices( Map<String, Map<String,Object>> data ) throws Exception {
		Writer writer = makeWriter();	
		...
		for( String thePk : data.keySet() ) {
			...
			if( invoiceJPA != null ) {
				...
				requestType.setRequestID("update_invoice_" + thePk);
				InvoiceMod invoice = factory.createInvoiceMod();
				
				invoice.setTxnID( (String) theseParms.get("TXNId"));
				invoice.setEditSequence( (String) theseParms.get("EditSequence") );
				...
				for( InvoiceItem item : invoiceJPA.getItems() ) {
					...
					lineItem.getItemRef().setFullName("Contractor Membership");
