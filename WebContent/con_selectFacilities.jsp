...
<%
	...
	boolean removeContractor = ("Remove".equals(request
			.getParameter("action")));
	if (isSubmitted) {
		...
		if (cBean.writeGeneralContractorsToDB(pBean, FACILITIES)) {
			...
			pcBean.generateDynamicCategories(id,
		...
		if (permissions.isContractor()) {
			response.sendRedirect("pqf_editMain.jsp?auditType="
					+ com.picsauditing.PICS.pqf.Constants.PQF_TYPE
	...
	if (permissions.isAdmin() && removeContractor) {
		com.picsauditing.PICS.pqf.CategoryBean pcBean = new com.picsauditing.PICS.pqf.CategoryBean();
		com.picsauditing.PICS.pqf.DataBean pdBean = new com.picsauditing.PICS.pqf.DataBean();
		Integer removeOpID = Integer.parseInt(request
				.getParameter("opID"));
		oBean.removeSubContractor(removeOpID, id);
		AccountBean tempOpBean = new AccountBean();
		tempOpBean.setFromDB(removeOpID.toString());
		cBean.addNote(id, "(" + pBean.userName + " from PICS)",
				"Removed " + aBean.name + " from " + tempOpBean.name
						+ "'s db", DateBean.getTodaysDateTime());
		pcBean.generateDynamicCategories(id,
				com.picsauditing.PICS.pqf.Constants.PQF_TYPE,
				cBean.riskLevel);
		cBean.setPercentComplete(
				com.picsauditing.PICS.pqf.Constants.PQF_TYPE,
				pdBean.getPercentComplete(id,
						com.picsauditing.PICS.pqf.Constants.PQF_TYPE));
		cBean.writeToDB();
	}//if
	cBean.setFromDB(id);
...
<table width="0" border="0" cellspacing="0" cellpadding="1">
	<tr>
		...
		%> <%@ include
			file="includes/pricing_matrix.jsp"%> <br>
		<b>Please select all facilities where you work<br>
		(Currently <%=cBean.facilitiesCount%>) </b><br>
		<br>
		<%
			}
			if (permissions.isAuditor()) {
		%> <strong><%=aBean.name%>'s</strong>
		facilities:<br>
		<br>
		<%
			}//if
			if (permissions.isAdmin() || permissions.isCorporate()) {
		%> Assign <strong><%=aBean.name%></strong>
		to the following facilities:<br>
		<br>
		<%
			}//if
		%>
		<table border="0" cellpadding="1" cellspacing="1">
			...
			<%
				...
				for (java.util.ListIterator<String> li = operators.listIterator(); li
						.hasNext();) {
					String opID = li.next();
					String name = li.next();
					String status = "";
					if (cBean.generalContractors.contains(opID)) {
						...
						if (permissions.isCorporate()
								&& !pBean.oBean.facilitiesAL.contains(opID)) {
			%>
			...
			<tr class="blueMain" <%=Utilities.getBGColor(count++)%>>
				...
				<%
					if (permissions.isAdmin()) {
				%> <a
					href=con_selectFacilities.jsp?id= <%=id%>&action=Remove&opID=
					<%=opID%>>Remove</a> <%
 	}
 %>
				</td>
			...
			<tr class=blueMain <%=Utilities.getBGColor(count++)%>>
				...
				%> <%=Inputs.getCheckBoxInput("genID_" + opID,
											"forms", "", "Yes")%>
				<%
					}
				%>
 ...
 %>
	...
	%>
	<tr>
		<td align="center"><input name="submit" type="image"
