	...
	<tbody>
		<s:iterator value="dataCriteria">
			<tr>
				...
				<td>
					<s:if test="get('daysFromExpiration') > 0"><s:text name="%{scope}.message.Added"><s:param><s:date name="get('effectiveDate')" format="MMM d" /></s:param></s:text></s:if>
					<s:else><s:text name="%{scope}.message.Removed"><s:param><s:date name="get('expirationDate')" format="MMM d" /></s:param></s:text></s:else>
				</td>
			</tr>
		</s:iterator>
	</tbody>
