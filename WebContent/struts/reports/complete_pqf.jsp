...
<s:form id="assignScheduleAuditsForm" method="post" cssClass="forms">
	...
	<div><input type="checkbox" onClick="toggle(this)" /> Check All
	</div>
	<table class="report">
		<thead>
			<tr>
				...
				<td><a>Audit</a></td>
				<td align="center"><a
					href="javascript: changeOrderBy('form1','createdDate ASC');">Created</a></td>
				<td align="center"><a
					href="javascript: changeOrderBy('form1','cao.percentComplete ASC');">%Complete</a></td>
				<td>Contacted</td>
				<td><a
					href="javascript: changeOrderBy('form1','scheduledDate ASC');">FollowUp</a></td>
			</tr>
		</thead>
		<s:iterator value="data" status="stat">
			<tr id="audit_<s:property value="get('auditID')"/>">
				...
				<td align="center"><s:checkbox name="sendMail"
					fieldValue="%{get('auditID')}" /></td>
				<td><a
					href="ContractorView.action?id=<s:property value="get('id')"/>"><s:property
					value="[0].get('name')" /></a></td>
				<td><a
					href="Audit.action?auditID=<s:property value="[0].get('auditID')"/>"><s:text
					name="%{[0].get('atype.name')}" /></a></td>
				...
				<a id="anchor<s:property value="[0].get('auditID')"/>"
					name="anchor<s:property value="[0].get('auditID')"/>" href="#"
					onclick="cal1.select($('scheduledDate[<s:property value="[0].get('auditID')"/>]'),'anchor<s:property value="[0].get('auditID')"/>','M/d/yy'); return false;"><img
					src="images/icon_calendar.gif" width="18" height="15" border="0" /></a>
				</td>
			</tr>
		</s:iterator>
	</table>
