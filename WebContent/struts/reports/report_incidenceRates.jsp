...
<s:form id="form1" name="form1" method="post">
            ...
            <table class="report">
              <thead><tr>
                ...
                <td>Location</td>
                <td>Type</td>
                <td><s:property value="year-1" /><s:checkbox name="searchYear1" /></td>
                <td><s:property value="year-2" /><s:checkbox name="searchYear2" /></td>
                <td><s:property value="year-3" /><s:checkbox name="searchYear3" /></td>
  			  </tr></thead>
			...
			<s:iterator value="data" status="stat">
			  <tr>
			    ...
			    <td>
			    <s:if test="%{[0].get('location') == 'Corporate'}">
			    <s:property value="[0].get('location')"/>
			    </s:if>
			    <s:else>
			    <s:property value="%{[0].get('location')+'-'+[0].get('description')}"/>
			    </s:else>
			    </td>
			    <td><s:property value="[0].get('SHAType')"/></td>
			   <!--Need to fix this before the year end-->	
			    <td class="right"><s:property value="%{new java.text.DecimalFormat('#,##0.00').format([0].get('recordableTotal1')*200000f/[0].get('manHours1'))}"/></td>
			    <td class="right"><s:property value="%{new java.text.DecimalFormat('#,##0.00').format([0].get('recordableTotal2')*200000f/[0].get('manHours2'))}"/></td>
			    <td class="right"><s:property value="%{new java.text.DecimalFormat('#,##0.00').format([0].get('recordableTotal3')*200000f/[0].get('manHours3'))}"/></td>
		  	  </tr>
			</s:iterator>
		    </table><br>
		    </s:form>
		    <center>
<s:property value="report.pageLinksWithDynamicForm" escape="false"/></center>
</body>
</html>