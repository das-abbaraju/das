...
value="%{new java.text.DecimalFormat('#,##0.00').format([0].get('recordableTotal')*200000f/[0].get('manHours'))}" escape="false"/>,"<s:property 
value="get('SHAType')"/>",<s:if test="%{[0].get('location') == 'Corporate'}">"<s:property 
value="[0].get('location')" />"</s:if><s:else>"<s:property
value="%{[0].get('location')+'-'+[0].get('description')}" />"</s:else>,<s:if test="permissions.operator">"<s:property 
