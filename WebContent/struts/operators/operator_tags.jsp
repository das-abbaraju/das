		...
		<table class="report">
			<thead>
				<tr>
					...
					<th><s:text name="OperatorTags.header.VisibleTo"><s:param value="%{getText('global.Operator')}" /></s:text></th>
			...
			<s:iterator value="tags" status="rowstatus">
				<tr><s:hidden name="tags[%{#rowstatus.index}].id" value="%{id}" />
					...
					<pics:permission perm="ContractorTags" type="Delete">
						...
						<s:if test="operator.id != permissions.accountId">
							<td>
								<p title="<s:text name="OperatorTags.TagBelongsToAnotherOperator">
											<s:param value="operator"/>
										  </s:text>">
						...
						<td id="${id}">
							<s:if test="permissions.isCanAddRuleForOperator(operator)">
								<s:if test="!auditTypeRules.empty">
									<p><a href="#" class="edit editAuditTypeRules">Audit Type Rules</a></p>
								</s:if>
								<s:if test="!auditCategoryRules.empty">
									<p><a href="#" class="edit editCategoryTypeRules">Category Rules</a></p>
								</s:if>
							</s:if>
							<s:if test="permissions.hasPermission('ManageAudits') && !operatorFlagCriteria.empty">
								<p><a href="ManageFlagCriteriaOperator.action?id=<s:property value="operatorFlagCriteria.get(0).operator.id" />" class="edit">Operator Flag Criteria</a></p>
							</s:if>
						</td>
					</s:if>
				</tr>
