						...
						<s:iterator value="yearsDescending" var="audit_group_year">
							<s:iterator value="distinctAuditTypes" var="audit_group_type">
								<s:if test="auditsByYearAndType.get(#audit_group_year, #audit_group_type) != null">
									<s:url action="EmployeeDashboard" method="employeeGUARDAudits"
									       var="employee_audit_group">
										<s:param name="id">
											${contractor.id}
										</s:param>
							...
							<tr>
								...
								<td>
									<s:if test="#audit.effectiveDate !=null">
										<s:date name="#audit.effectiveDate"
										        format="%{@com.picsauditing.util.PicsDateFormat@Iso}"/>
									</s:if>
								</td>
								<td>
									<s:if test="#audit.operators.size > 0">
										<s:iterator value="#audit.getCaoStats(permissions).keySet()" id="status"
										            status="stat">
								...
								<td>
									<s:if test="#audit.hasOpenRequirements">
										<a href="ContractorAuditFileUpload.action?auditID=<s:property value='#audit.id'/>"><s:text
												name="EmployeeDashboard.Upload"/></a>
									</s:if>
								</td>
							</tr>
						</s:iterator>
				...
				<s:if test="activeEmployees.size() > 0">
					<table class="table">
						...
						<tr>
							...
							<th>
								<s:text name="EmployeeDashboard.SkillsTraining"/>
							</th>
						</tr>
						</thead>
                        <s:iterator value="activeEmployees" var="employee" status="stat">
                            ...
                            <tr>
								...
								<td>
									<s:set var="overall_employee_file_status" value="%{#employee.overallFileStatus}"/>
									<s:if test="#overall_employee_file_status.notApplicable">
										<s:text name="%{#employee.overallFileStatus.i18nKey}"/>
									</s:if>
									<s:else>
										...
										<a href="${employee_skills_training}">
											<s:text name="%{#employee.overallFileStatus.i18nKey}"/>
										</a>
									</s:else>
								</td>
						...
						<tr>
							...
							<td>
								<s:iterator value="#role.jobCompetencies" var="jobCompetency" status="stat">
									<s:property value="#jobCompetency.competency.label"/><s:if
										test="!#stat.last">, </s:if>
								</s:iterator>
							</td>
						</tr>
