...
<s:if test="employee.account.requiresCompetencyReview">
	<fieldset class="form">
		<h2 class="formLegend"><s:text name="ManageEmployees.header.HSESites" /></h2>
		<ol>
			<li>
				<table class="report" style="width: 500px;">
					<thead>
						<tr>
							...
							<th><s:text name="ManageEmployees.label.Since" /></th>
							<th><s:text name="ManageEmployees.label.Orientation" /></th>
					...
					<s:iterator value="employee.employeeSites" id="site" status="stat">
						<s:if test="#site.current && #site.jobSite == null">
							<tr>
								...
								<td class="center"><a href="#" id="getSite_<s:property value="id" />" class="edit getSite"></a>
								</td>
							</tr>
						</s:if>
					</s:iterator>
					<tr>
						<td colspan="4">
							<s:if test="hseOperators.size > 0">
								<s:select list="hseOperators" listKey="id" listValue="name" headerKey="" 
									headerValue=" - %{getText('ManageEmployees.header.AssignSite')} - "
									id="hseOperator" />
							</s:if>
							<s:else>
								<h5><s:text name="ManageEmployees.message.AssignedAllHSESites" /></h5>
							</s:else>
						</td>
					</tr>
				</table>
				<s:if test="hseOperators.size > 0">
					<div class="fieldhelp" title="<s:text name="ManageEmployees.label.AddHSESite" />">
						<p><s:text name="ManageEmployees.label.AddHSESite.fieldhelp" /></p>
					</div>
				</s:if>
			</li>
		</ol>
...
<s:if test="employee.account.requiresOQ">
	<fieldset class="form">
		<h2 class="formLegend"><s:text name="ManageEmployees.header.OQProjects" /></h2>
		<ol>
			<li>
				<table class="report" style="width: 500px;">
					<thead>
						<tr>
							<th><s:text name="global.Operator" /></th>
							<th><s:text name="ManageEmployees.label.Project" /></th>
							<th><s:text name="ManageEmployees.label.Since" /></th>
							<th><s:text name="button.Edit" /></th>
							<th><s:text name="ManageEmployees.label.Tasks" /></th>
						</tr>
					</thead>
					...
					<s:iterator value="employee.employeeSites" id="site" status="stat">
						<s:if test="#site.current && #site.jobSite != null">
							...
							<tr>
								...
								<td class="center"><a href="#" id="getOQSite_<s:property value="id" />" class="edit getSite"></a>
								</td>
								<td class="center">
									<a href="#" class="preview qualifiedTasks" id="statCount_<s:property value="#stat.count" />"></a>
								</td>
							</tr>
							<tr id="<s:property value="#stat.count" />_tasks" class="qualifiedTasks" style="display: none;">
								<td colspan="5" style="padding: 10px">
									...
									<h4><s:text name="ManageEmployees.label.QualifiedTasks" /></h4>
										...
										<table class="report">
											<thead>
												<tr>
													<th><s:text name="JobTask.label" /></th>
													<th><s:text name="JobTask.name" /></th>
													<th><s:text name="ManageEmployees.label.RecommendedAssessmentTests" /></th>
												</tr>
											</thead>
											<tbody>
													...
													<tr>
														...
														<td>
															<s:iterator value="#mt.jobTaskCriteriaMap.keySet()" var="mtGroupNum" status="mtGroup">
																...
																<s:if test="!#mtGroup.last"><s:text name="ManageJobTaskCriteria.separator.OR" /><br /></s:if>
															</s:iterator>
														</td>
													</tr>
												</s:iterator>
									...
									<s:if test="#site.jobSite.tasks.size == 0">
										<h5><s:text name="ManageEmployees.message.SiteHasNoJobTasks" /></h5><br />
									</s:if>
									<a href="#" class="remove closeSiteTasks" id="closeSiteTask_<s:property value="#stat.count" />">
					...
					<s:if test="#oqSiteCount == 0">
						<tr>
							<td colspan="5"><s:text name="ManageEmployees.message.NoAssignedProjects" /></td>
						</tr>
					</s:if>
				</table>
			</li>
			<li>
				<s:if test="oqOperators.size > 0">
					<s:select list="oqOperators" listKey="id" listValue="name" headerKey="" id="oqOperator"
						headerValue=" - %{getText('ManageEmployees.header.AssignProject')} - " />
				</s:if>
			</li>
			<pics:permission perm="ManageProjects" type="Edit">
				<li>
					<a class="add" href="#" id="newJobSiteLink"><s:text name="ManageEmployees.link.AddNewJobSite" /></a>
				</li>
				<div id="newJobSiteForm">
					<h4><s:text name="ManageEmployees.link.AddNewJobSite" /></h4>
					<s:if test="permissions.admin && employee.account.contractor">
						<li><label><s:text name="global.Operator" />:</label>
					...
					<li class="required"><s:textfield name="jobSite.label" maxlength="15" theme="formhelp" /></li>
					<li class="required"><s:textfield name="jobSite.name" maxlength="255" theme="formhelp" /></li>
					<li><s:textfield name="jobSite.projectStart" cssClass="datepicker" value="%{today}" theme="formhelp" /></li>
					<li><s:textfield name="jobSite.projectStop" cssClass="datepicker" value="%{expirationDate}" theme="formhelp" /></li>
					<li>
						<a href="#" class="picsbutton positive"><s:text name="ManageEmployees.button.SaveNewJobSite" /></a>
						<a href="#" class="picsbutton cancelButton"><s:text name="button.Cancel" /></a>
					</li>
				</div>
			</pics:permission>
			<s:if test="employee.account.contractor">
				<li><a href="ReportNewProjects.action" class="add"><s:text name="ManageEmployees.link.FindNewProjects" /></a></li>
			</s:if>
		</ol>
	</fieldset>
</s:if>
<s:if test="employee.prevAssigned">
	<fieldset class="form">
		<h2 class="formLegend"><s:text name="ManageEmployees.header.PreviouslyAssignedSitesProjects" /></h2>
		<ol>
			<li>
				<table class="report">
					<thead>
					...
					<s:iterator value="employee.employeeSites" id="site" status="stat">
						<s:if test="!#site.current"> 
							<tr>
								<td>
									<s:if test="jobSite.id > 0"><s:text name="ManageEmployees.header.OQProjects" /></s:if>
									<s:else><s:text name="ManageEmployees.header.HSESites" /></s:else>
								</td>
								<td><s:property value="operator.name" /></td>
								<td>
