...
<s:form cssClass="form">
	...
	<fieldset>
		...
		<ol>
			...
			<s:else>
				<s:hidden id="u" name="u" value="%{u.id}" />
				<s:hidden name="user" value="%{user.id}" />
				<li><s:password name="passwordc" label="global.CurrentPassword"
						theme="form" /></li>
			</s:else>
			<li><s:password name="password1" label="global.Password"
					theme="form" /></li>
			<li><s:password name="password2"
					label="ProfileEdit.ConfirmPassword" theme="form" /></li>
		</ol>
	</fieldset>
	<fieldset class="form submit">
		...
		<s:if test="source=='manage' && user.id != u.id">
			<s:submit value="%{getText('UsersManage.SendResetPasswordEmail')}"
			cssClass="picsbutton positive" method="emailPassword" />			
		</s:if>
	</fieldset>
