...
<ul id="navlist">
	<li><a id="link_tab_profile" href="#" class="current"><s:text name="button.Edit" /></a></li>
	<s:if test="switchTos.size > 0">
		<li><a id="link_tab_switch" href="#"><s:text name="ProfileEdit.label.SwitchAccounts" /></a></li>
	</s:if>
	<s:if test="eList.size > 0">
		<li><a id="link_tab_subscriptions" href="#"><s:text name="ProfileEdit.label.EmailSubscriptions" /></a></li>
	</s:if>
	<s:if test="!permissions.contractor">
		<li><a id="link_tab_permissions" href="#"><s:text name="ProfileEdit.label.Permissions" /></a></li>
	</s:if>
</ul>
</div>
...
<div class="alert"><s:text name="ProfileEdit.alert.ChangePasswordRequired" /></div>
</s:if>
...
<s:form id="saveProfileForm" cssClass="form">
	...
	<h2><s:text name="ProfileEdit.Profile.heading" /></h2>
		<ol>
			<li>
				<label><s:text name="ProfileEdit.AssignedToAccount"></s:text>:</label> <s:property value="u.account.name" />
			</li>
			<li>
		...
		<s:iterator value="recentLogins">
			<tr>
				...
				<td>
					...
					<s:if test="successful == 'N'"><s:text name="ProfileEdit.message.IncorrectPasswordAttempt" /></s:if>
				</td>
			</tr>
		</s:iterator>
		</tbody>
...
<div id="tab_switch" style="display: none;">
	<h3><s:text name="ProfileEdit.label.SwitchAccounts" /></h3>
	<table class="report">
		<thead>
			<tr>
...
<table style="width: 100%">
	<tr>
		<td>
		<h3><s:text name="ProfileEdit.label.Permissions" /></h3>
		<table class="report">
			<thead>
				<tr>
			...
			<s:iterator value="permissions.permissions">
				<tr>
					...
					<td><s:if test="viewFlag"><s:text name="OpType.View" /></s:if></td>
					<td><s:if test="editFlag"><s:text name="OpType.Edit" /></s:if></td>
					<td><s:if test="deleteFlag"><s:text name="OpType.Delete" /></s:if></td>
				</tr>
			</s:iterator>
			</tbody>
		</table>
		...
		<td>
			<h3><s:text name="ProfileEdit.header.VisibleAuditAndPolicyTypes" /></h3>
			<div>
			...
			<s:else><s:text name="ProfileEdit.message.YouAreAPICSEmployee" /></s:else>
			</div>
			<s:if test="permissions.admin && permissions.shadowedUserID != permissions.userId">
				<h3><s:text name="ProfileEdit.header.Shadowing" /></h3>
				<div><s:text name="ProfileEdit.message.YouAreCurrentlyShadowing"><s:param value="%{permissions.shadowedUserName}" /></s:text></div>
			</s:if>
		</td>
	</tr>
</table>
