...
<body>
	...
	<s:if test="user.id == 0">&gt; <s:text name="UsersManage.NewUser" /></s:if>
	...
	<div id="user_edit">
		...
		<s:if test="user != null">
			<div style="margin-bottom: 10px;">
				<button class="picsbutton" onclick="showUserList();">
					...
					<div class="alert">
						...
						<s:if test="permissions.admin">
							<span title="<s:text name="UsersManage.PressButtonToUnlock" />">
								<s:submit method="unlock" cssClass="picsbutton negative" value="%{getText('UsersManage.UnlockThisAccount')}" />
							</span>
						</s:if>
					</div>
				</s:if>
				...
				<fieldset class="form">
					...
					<ol>
						<s:if test="account.users.size() > 1">
							<s:if test="user.id > 0">
								...
								<li>
									...
									<s:date name="user.creationDate" format="%{getText('date.short')}" />
								</li>
							</s:if>
						</s:if>
						...
						<s:if test="user.isGroup.toString() == 'No'">
							...
							<li>
								<label>
									<s:text name="UsersManage.ManuallySetPassword" />
								</label> 
							...
							<li class="user-password">
								<label>
									<s:text name="global.ConfirmPassword" />
								</label>
							...
							<s:if test="account.contractor">
								<li>
									<label>
										<s:text name="UsersManage.UserRole" />
									</label>
							...
							<s:if test="csr && permissions.admin">
								<li>
									<label>
										<s:text name="UsersManage.ShadowCSR" />
									</label>
									<s:select
										list="csrs"
										listKey="user.id"
						...
						<s:if test="user.id > 0">
							...
							<s:if test="permissions.isAdmin()">
								...
								<li>
									<label>
										<s:text name="UsersManage.MoveUserToAccount" />
									</label>
							...
							<s:if test="hasProfileEdit">
								<li>
									<label>&nbsp;</label>
									<a class="picsbutton" href="?button=resetPassword&user=<s:property value="user.id"/>">
						...
						<tbody>
							<s:iterator value="recentLogins">
								<tr>
									...
									<td>
										<a href="http://www.hostip.info/?spip=<s:property value="remoteAddress" />">
