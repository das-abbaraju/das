	...
	<body>
		...
		<div id="cluetip2">
			<s:text name="ContractorFlag.MinimumOperatorRequirements">
				<s:param><s:property value="co.operatorAccount.name"/></s:param>
			</s:text>
		</div>
		<div id="info-box">
			<div class="baseFlag">
					...
					<s:if test="contractor.lastRecalculation != null">
						<s:if test="permissions.admin || permissions.operatorCorporate">
							<s:text name="ContractorFlag.ContractorLastCalculated">
								<s:param><s:date name="contractor.lastRecalculation" nice="true" /></s:param>
							</s:text>
						</s:if>
						<s:else>
				...
				<div class="panel">
					<div class="panel_header">
						...
						<span style="float: right;">
							<a href="#" onclick="return false;" class="cluetip help" rel="#cluetip2" title="<s:text name="ContractorFlag.FlagHelp" />"></a>
						</span>
					</div>
								...
								<s:if test="canForceOverallFlag(co.forceOverallFlag)">
									<s:form cssStyle="border: 2px solid #A84D10; background-color: #FFC; padding: 10px;">
										...
										<s:if test="co.forceOverallFlag.operatorAccount.type == 'Corporate'">
											...
											<label><s:text name="ContractorFlag.CancelOverride"></s:text></label>
											<br/>
										</s:if>
									...
									<div id="override" style="display: none">
										<s:form id="form_override" enctype="multipart/form-data" method="POST">
											...
											<ol>
												...
												<li> 
													<span class="label-txt">
														<s:text name="ContractorFlag.Until"></s:text>
													</span> 
												...
												<li class="required">
													...
													<s:textarea name="forceNote" value="" rows="2" cols="15" cssStyle="vertical-align: top;"></s:textarea>
													<br />
													<div class="fieldhelp">
												...
												<li>
													<span class="label-txt">
														<s:text name="ContractorFlag.FileAttachment"></s:text>
													</span>
													...
													<s:if test="permissions.corporate">
														...
														<label for="overRAll_main">
															<s:text name="ContractorFlag.OverrideForAllSites">
																<s:param><s:property value="permissions.accountName" /></s:param>
															</s:text>
														</label>
														<br/>
													</s:if>
		...
		<div id="tabs">
			<ul>
				<li>
					<a href="#tabs-1"><s:text name="ContractorFlag.Problems"></s:text></a>
				</li>
				<li>
					<a href="#tabs-2"><s:text name="ContractorFlag.FlagableData"></s:text></a>
				</li>
				<li>
					<a href="#tabs-3"><s:text name="ContractorFlag.Notes"></s:text></a>
				</li>
			</ul>
				...
				<s:if test="displayTable">
					<table class="report" style="clear: none;">
						<thead>
							<tr>
								<td>
									<s:text name="ContractorFlag.Flag"></s:text>
								</td>
								<td>
									<s:text name="ContractorFlag.Description"></s:text>
								</td>
								<td>
									<s:text name="ContractorFlag.Value"></s:text>
								</td>
								<td>
									<s:text name="ContractorFlag.OverrideDetail"></s:text>
								</td>
							</tr>
						</thead>
						<tbody>
											...
											<td>
												...
												<s:if test="#opCriteria != null">
													<s:if test="#data.criteria.oshaType != null || (#data.criteria.question != null && #data.criteria.question.id == 2034)">
														...
														<a href="#" onclick="return false;" class="help cluetip" rel="#cluetip<s:property value="#opCriteria.id" />" title="<s:text name="ContractorFlag.Statistics"></s:text>"></a>
												...
												<s:if test="#flagoverride != null">
													...
													<div id="cluetip<s:property value="#flagoverride.id"/>">
														<span title='<s:text name="ContractorFlag.ByTitle"><s:param><s:property value="#flagoverride.updatedBy.name" /></s:param><s:param><s:property value="#flagoverride.updatedBy.account.name"/></s:param><s:param><s:if test="#flagoverride.updatedBy.account.corporate"> <s:text name="ContractorFlag.ForAllSites" /></s:if></s:param></s:text>'>
															...
															<s:text name="ContractorFlag.Until"></s:text>
															<s:date name="#flagoverride.forceEnd" format="MMM d, yyyy" />
												...
												<td colspan="3">
													<div class="override_form">
														<form enctype="multipart/form-data" method="POST">
															...
															<s:if test="canForceDataFlag(#flagoverride)">
																<s:if test="#flagoverride!=null">
																	<s:text name="ContractorFlag.SiteOverride">
																		<s:param><s:property value="#flagoverride.createdBy.name"/></s:param>
																	</s:text>
																</s:if>
																<s:else>												
																	<fieldset class="form">
															...
															<s:else>
																<s:if test="#flagoverride.operator.type == 'Corporate'">
																	...
																	<label><s:text name="ContractorFlag.CancelOverrideInfo"></s:text></label>
																	<br />
																</s:if>
																&nbsp;
														...
														<span class="override_wrap override_hide">
															<a href="#" class="override_wrench override_table_link"><s:text name="ContractorFlag.HideForceLine"></s:text></a>
														</span>
													</div>
													
													<span class="override_wrap">
														<a href="#" class="override_wrench override_table_link"><s:text name="ContractorFlag.ShowForceLine"></s:text></a>
													</span>
												</td>
											</tr>
										</pics:permission>
