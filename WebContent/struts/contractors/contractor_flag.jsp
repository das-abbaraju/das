...
<html>
<head>
<title><s:text name="ContractorFlag.title">
		<s:param>
...
.clickable .override_wrap {
	display: inline;
}

.override_form .override_wrap {
	display: inline;
}
</style>
...
<body>
	...
	<div id="info-box">
		<div class="baseFlag">
			<s:if
				test="permissions.picsEmployee && (co.flagColor != co.baselineFlag)">
				<div class="alert">
					<s:form>
						<s:hidden name="id" />
						<s:hidden name="opID" />
						...
						<s:text name="ContractorFlag.FlagRecentlyChanged">
							<s:param>
								<s:property
									value="@com.picsauditing.jpa.entities.FlagColor@getSmallIcon(co.baselineFlag)"
									escape="false" />
							</s:param>
							<s:param>
							...
							<s:param>
								<s:property
									value="@com.picsauditing.jpa.entities.FlagColor@getSmallIcon(co.flagColor)"
									escape="false" />
							</s:param>
							<s:param>
						...
						<s:submit type="button" method="approveFlag"
		...
		<div class="info">
			<s:form>
				...
				<s:submit type="button" name="button" method="recalculate"
		...
		<div class="panel_placeholder">
			<div class="panel">
				<div class="panel_header">
					<s:text name="ContractorFlag.FlagStatus"></s:text>
					<span style="float: right;"> <a href="#"
						onclick="return false;" class="cluetip help" rel="#cluetip2"
						title="<s:text name="ContractorFlag.FlagHelp" />"></a> </span>
				</div>
				<div class="panel_content">
					...
					<div class="FlagCriteriaContent">
						<b> <pics:permission perm="EditFlagCriteria">
								<a
									href="ManageFlagCriteriaOperator.action?id=<s:property value="co.operatorAccount.inheritFlagCriteria.id" />"
									title="<s:text name="ContractorFlag.ViewFlagCriteria" />"><s:property
										value="co.operatorAccount.name" />
								</a>
							</pics:permission> <pics:permission perm="EditFlagCriteria" negativeCheck="true">
								<s:property value="co.operatorAccount.name" />
							</pics:permission> </b> <br />
							...
							<s:form
								...
								<s:if test="co.forceOverallFlag != null">
									<s:text name="ContractorFlag.ManualForceFlagInfo">
										<s:param>
											<s:property value="co.forceOverallFlag.forceFlag.smallIcon"
												escape="false" />
										</s:param>
										<s:param>
										...
										<s:param>
											<s:if
												test="co.forceOverallFlag.operatorAccount.type == 'Corporate'">
								...
								<a
									href='ContractorNotes.action?id=<s:property value="contractor.id"/>&amp;filter.category=Flags&amp;filter.keyword=Forced'><s:text
										name="ContractorFlag.ViewNotes" />
								</a>
							</s:form>
						</s:if>
						<br />
						...
						<pics:permission perm="EditForcedFlags">
							<s:if test="canForceOverallFlag(co.forceOverallFlag) && co.forceOverallFlag.operatorAccount.type == 'Corporate' && isDisplayCorporate()">
								<s:form
									...
									<div>
										<s:submit type="button" name="button" method="cancelOverride"
							...
							<s:else>
								<div id="override" style="display: none">
									<s:form id="form_override" enctype="multipart/form-data"
										method="POST">
										<s:hidden name="id" />
										<s:hidden name="opID" />
										<fieldset class="form">
											...
											<ol>
												...
												<li><span class="label-txt"> <s:text
													...
													class="datepicker" /></li>
												<li class="required"><span class="label-txt"> <s:text
															name="ContractorFlag.Reason"></s:text>: </span> <s:textarea
														name="forceNote" value="" rows="2" cols="15"
														cssStyle="vertical-align: top;"></s:textarea> <br />
													<div class="fieldhelp">
														<h3>
									...
									<a href="#"
										onclick="$('#override_link').slideDown(); $('#override').slideUp(); return false;"><s:text
											name="ContractorFlag.Nevermind"></s:text>
									</a>
								</div>
	...
	<s:if
		test="co.flagColor.toString() == 'Green' && co.forceOverallFlag == null && flagDataOverrides.keySet().size() == 0">
	...
	<div id="tabs">
		<ul>
			<li><a href="#tabs-1"><s:text name="ContractorFlag.Problems"></s:text>
			</a></li>
			<li><a href="#tabs-2"><s:text
						name="ContractorFlag.FlagableData"></s:text>
			</a></li>
			<li><a href="#tabs-3"><s:text name="ContractorFlag.Notes"></s:text>
			</a></li>
		</ul>
							...
							<s:iterator id="data" value="flagDataMap.get(#key)">
								<s:if test="#data.flag.redAmber || isFlagDataOverride(#data, #data.operator)">
									...
									<tr class="<s:property value="#data.flag" />">
										...
										<td><s:set name="opCriteria"
												...
												<s:if
													...
													<a href="#" onclick="return false;" class="help cluetip"
														...
														title="<s:text name="ContractorFlag.Statistics"></s:text>"></a>
													<div id="cluetip<s:property value="#opCriteria.id" />">
														<s:text name="ContractorFlag.StatusNotPending"></s:text>
													</div>
												</s:if>
												<s:else>
													<s:property value="#opCriteria.replaceHurdle" />
										...
										<!-- Value -->
										<td><s:if test="#data.criteria.auditType != null">
												<s:iterator
													value="missingAudits.get(#data.criteria.auditType)"
													var="cao">
												...
												<s:if test="#data.criteria.auditType.classType.policy">
													...
													<p style="padding-left: 20px; font-size: x-small">
														<s:if test="#cao.flag != null">
															<s:text name="ContractorFlag.Recommend"></s:text>
														...
														<s:iterator var="insuranceFlagData" value="co.flagDatas">
															<s:if
																test="#insuranceFlagData.criteria.insurance && #insuranceFlagData.criteria.question.auditType == #data.criteria.auditType">
																<s:iterator id="conCriteria"
																	value="contractor.flagCriteria">
																	<s:if
																		test="#insuranceFlagData.criteria.id == #conCriteria.criteria.id">
																		<s:property
																			value="getContractorAnswer(#conCriteria, #insuranceFlagData, true)"
																			escape="false" />
																		<br />
																	</s:if>
																</s:iterator>
												...
												<s:iterator id="conCriteria" value="contractor.flagCriteria">
													<s:if test="#data.criteria.id == #conCriteria.criteria.id">
														<s:property
															value="getContractorAnswer(#conCriteria, #data, false)"
															escape="false" />
													</s:if>
												</s:iterator>
											</s:else></td>
										...
										<!-- Override Detail -->
										<td class="center"><s:if test="#flagoverride != null">
												<a href="#" onclick="return false;" class="cluetip help"
													rel="#cluetip<s:property value="#flagoverride.id"/>"
													title="<s:text name="ContractorFlag.Location"></s:text>"><s:text
														name="ContractorFlag.ManualForceFlag"></s:text>
												</a>
						...
						<td>
							<table class="report">
								...
								<s:iterator id="data" value="#flagData.value">
									<tr>
										<td class="center"><s:property value="flag.smallIcon"
												escape="false" /></td>
										<td><s:if test="criteria.auditType != null">
		...
		<div id="tabs-3">
			<s:if
				test="co.operatorAccount.approvesRelationships.toString() == 'Yes'">
				<s:if test="co.workStatusPending">
					<div class="alert">
						<s:text name="ContractorFlag.OperatorHasNotApproved"></s:text>
					</div>
				</s:if>

				<s:if test="co.workStatusRejected">
					<div class="alert">
						<s:text name="ContractorFlag.OperatorDidNotApproved"></s:text>
					</div>
				</s:if>
			</s:if>
