	...
	<body>
		...
		<s:if test="permissions.admin && payment.qbSync">
			<div class="alert" class="noprint">
				This payment is still waiting to be synced with QuickBooks!
			</div>
		</s:if>
		...
		<s:if test="payment == null && method.creditCard">
			<s:iterator value="contractor.payments">
				<s:if test="status.unpaid">
					<div class="alert">
						This contractor still has an <a href="PaymentDetail.action?payment.id=<s:property value="id"/>">open credit</a>. Please apply that credit before collecting any new payments.
					</div>
				</s:if>
			</s:iterator>
		</s:if>
		...
		<s:form onsubmit="cleanPaymentMethods()">
							...
							<td style="vertical-align: top;" valign="top">
								<ol>
									...
									<li>
										...
										<s:property value="payment.currency"/>
										
										<s:hidden id="payment_totalAmount" value="%{payment.totalAmount}"/>
									</li>
									<s:if test="payment.status.unpaid">
										<li>
											<label>Applied:</label>
											<span id="payment_amountApplied"><s:property value="payment.amountApplied"/></span>
											<s:property value="payment.currency"/>
										</li>
										<li>
											<label>Credit:</label>
											<span id="payment_balance"><s:property value="payment.balance"/></span>
											<s:property value="payment.currency"/>
										</li>
									</s:if>
								</ol>
								...
								<ol>
									...
									<s:if test="payment.paymentMethod.check">
										<li>
											...
											<s:textfield name="payment.checkNumber" />
										</li>
									</s:if>
									...
									<s:if test="payment.paymentMethod.creditCard">
										<li>
											...
											<s:property value="payment.transactionID" />
										</li>
										<li>
											<label>Type:</label>
										...
										<li>
											<label>Number:</label>
											<s:property value="payment.ccNumber" />
										</li>
										<li>
											<label>Status:</label>
											<s:property value="transactionCondition" />
										</li>
										
										<s:if test="transactionCondition == 'pendingsettlement'">
											<li>
												<label>Void CC:</label>
												<a class="remove" href="PaymentDetail.action?payment.id=<s:property value="payment.id" />&button=voidcc">Void</a>
											</li>
										</s:if>
									</s:if>
								</ol>
							...
							<td style="vertical-align: top;" valign="top">
								<ol>
									...
									<li>
										...
										<s:textfield id="payment_totalAmount" name="payment.totalAmount" value="0.00" size="7" onchange="changeTotal()"/> <s:property value="payment.currency"/>
									</li>
									<li>
										<label>Applied:</label>
										<span id="payment_amountApplied">0.00</span> <s:property value="payment.currency"/>
									</li>
									<li>
										<label>Credit:</label>
										<span id="payment_balance">0.00</span>
										<s:property value="payment.currency"/>
										<input type="button" value="Clear" onclick="calculateTotalFromApplied()">
									</li>
								</ol>
							</td>
							<td style="vertical-align: top;" valign="top">
								<ol id="paymentMethodList">
									...
									<li>
										...
										<s:checkbox name="changePaymentMethodOnAccount"/>
									</li>
									<li class="method_check" <s:if test="!method.check">style="display: none"</s:if>>
										<label>Check Number:</label>
										<s:textfield name="payment.checkNumber" />
									</li>
									<li>
									...
									<li class="method_ccNew" style="display: none">
										<label>Number:</label>
										<s:textfield name="creditCard.cardNumber" />
									</li>
									<li class="method_ccNew" style="display: none">
										<label>Expires:</label>
										<s:textfield name="creditCard.expirationDate" />
									</li>
								</ol>
							</td>
						</tr>
				...
				<table class="report" style="clear:none">
					...
					<s:iterator value="payment.invoices">
						<tr>
							...
							<td>
								<a class="remove" href="PaymentDetail.action?payment.id=<s:property value="payment.id" />&button=unapply&amountApplyMap[<s:property value="invoice.id" />]=0">Remove</a>
							</td>
						</tr>
					</s:iterator>
					...
					<s:iterator value="payment.refunds">
						<tr>
							...
							<td>
								<a class="remove" href="PaymentDetail.action?payment.id=<s:property value="payment.id" />&button=unapply&amountApplyMap[<s:property value="refund.id" />]=0">Remove</a>
							</td>
						</tr>
					</s:iterator>
				</table>
			...
			<s:if test="hasUnpaidInvoices">
					...
					<s:iterator value="contractor.invoices">
						<s:if test="totalAmount > 0 && status.unpaid">
							<tr>
								...
								<td class="right">
									<span id="invoice_balance_<s:property value="id"/>"><s:property value="balance" /></span>
									<s:property value="payment.currency"/>
								</td>
								<td>
									<input type="button" onclick="setInvoiceApply(<s:property value="id" />);" value="&gt;&gt;"/>
								</td>
								<td class="right">
									<s:textfield id="invoice_apply_%{id}" cssClass="amountApply" name="amountApplyMap[%{id}]" size="6" onchange="updateSingleAppliedAmount(%{id});" />
									<s:property value="payment.currency"/>
								</td>
							</tr>
						</s:if>
				...
				<s:if test="payment == null">
					<div>
						<input id="autoapply" type="checkbox" checked="checked" onchange="autoApply();">
