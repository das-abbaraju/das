...
<script type="text/javascript">
	...
	$(function() {
		...
		<s:iterator value="auditTypes.keySet()" var="tab" status="i">
			$('#internalnavcontainer a#<s:property value="#tab.jsName" escape="false" />').click(function() { $('#tabs').tabs('select', <s:property value="#i.index" />); return false; });
		</s:iterator>
	});
</script>
<style type="text/css">
...
<div id="tabs">
	<ul>
		<s:iterator value="auditTypes.keySet()" var="classType">
			<li><a href="<s:property value="'#' + #classType.safeName" />"><s:property value="#classType.name" escape="false" /></a></li>
		</s:iterator>
	</ul>
	<s:iterator value="auditTypes.keySet()" var="classType" status="stat">
		<div id="<s:property value="#classType.safeName" />">
			<table class="report" id="table_<s:property value="#classType.safeName" />">
				<thead>
					<tr>
						<th>Name</th>
				...
				<tbody>
							...
							<tr>
								<td>
									<a href="Audit.action?auditID=<s:property value="#audit.id" />">
										...
										<s:if test="#classType.safeName == 'Policy'"><br /><span style="font-size: 10px"><s:date name="#audit.effectiveDate" format="MMM yyyy" /></span></s:if>
									</a>
								</td>
											...
											<s:iterator value="#audit.operators" id="cao">
												<s:if test="#cao.isVisibleTo(permissions)">
													<tr>
														...
														<s:if test="#classType.safeName == 'IM'">
					...
					<tr>
						...
						<td colspan="<s:property value="#colspan" />" class="center">
							<a href="#" onclick="showAll('table_<s:property value="#classType.safeName" />'); return false;" class="preview">View All</a>
						</td>
					</tr>
				</tbody>
			</table>
			
			<s:if test="#classType.safeName == 'IM' && imScores.keySet().size > 0">
				<h3>Overall Integrity Management</h3>
				<table class="report">
					<thead>
					<tr>
						<th>Audit Name</th>
			...
			<s:if test="expiredAudits.get(#classType).size > 0">
				<h3>Expired</h3>
				<table class="report" id="expired_<s:property value="#classType.safeName" />">
					<thead>
					<tr>
						<th>Name</th>
						<th>Safety Professional</th>
					...
					<tr>
						<td colspan="<s:property value="#colspan" />" class="center">
							<a href="#" onclick="showAll('expired_<s:property value="#classType.safeName" />'); return false;" class="preview">View All</a>
						</td>
					</tr>
				</table>
			</s:if>
