	...
	try {
		...
		if ("Remove".equals(action)) {
			if (pBean.oBean.removeSubContractor(permissions.getAccountId(), actionID)) {
				...
				cBean.addNote(actionID, "", pBean.userName + " from " + aBean.name
						+ " removed contractor from its db", DateBean.getTodaysDateTime());
				cBean.writeToDB();
			}
		}
...
<table width="657" border="0" cellpadding="0" cellspacing="0">
	<tr>
		<td height="70" colspan="2" align="center" class="blueMain"><%@ include
			file="includes/nav/opSecondNav.jsp"%> <span
			class="blueHeader">Contractor List</span><br>
		<span class="redMain">You have <strong><%=pBean.getCanSeeSetCount()%></strong>
		contractors in your database.</span><br>
		...
		<table border="0" cellpadding="2" cellspacing="0">
			<tr align="center">
				<td><input name="name" type="text" class="forms"
					...
					onFocus="clearText(this)"> <%=tBean.getTradesSelect("trade", "forms", sBean.selected_trade)%>
				<%=Inputs.inputSelect("performedBy", "forms", sBean.selected_performedBy,
								TradesBean.PERFORMED_BY_ARRAY)%></td>
...
<table width="657" border="0" cellpadding="1" cellspacing="1">
	<tr bgcolor="#003366" class="whiteTitle">
		...
		<%
			if (permissions.isOperator()) {
		%>
		<td align="center" bgcolor="#6699CC"><a href="?orderBy=flag DESC"
			class="whiteTitle">Flag</a></td>
		<%
			}
		%>
		<%
	...
	<tr <%=sBean.getBGColor()%> class="<%=thisClass%>">
		...
		<td><a href="contractor_detail.jsp?id=<%=sBean.aBean.id%>"
			title="view <%=sBean.aBean.name%> details" class="<%=thisClass%>"><%=sBean.aBean.name%></a>
		</td>
		<%
			if (permissions.canSeeAudit(AuditType.PQF)) {
		%>
		...
		%> <a
			href="contractor_upload_certificates.jsp?id=<%=sBean.aBean.id %>">
 ...
 %> <img src=images/notOkCheck.gif width=19 height=15 alt='Non Uploaded'>
		...
		%>
		<td align="center"><a
			href="con_redFlags.jsp?id=<%=sBean.cBean.id%>"
			title="Click to view Flag Color details"><%=sBean.getFlagLink()%></a></td>
		<%
			}
		%>
		<%
