...
function showAuditTypeRules(tagID) {
	...
	$('#rules').think({message: translate('JS.OperatorTags.message.LoadingRelatedRules', ['<s:text name="AuditType" />']) }).load('AuditTypeRuleTableAjax.action', data);
}
...
function showCategoryRules(tagID) {
	...
	$('#rules').think({message: translate('JS.OperatorTags.message.LoadingRelatedRules', ['<s:text name="AuditCategory" />']) }).load('CategoryRuleTableAjax.action', data);
}