...
<body>
<h1>Insurance Approval <span class="sub">InsureGuard</span></h1>
...
<input name="s_certStatus" type="hidden"
...
<table id="certTable" class="report">
	...
	<tr id="<%=certDO.getCert_id()%>" <%=sBean.getBGColor()%>
		...
		%>
		<td align="center"><input name="status_<%=certDO.getCert_id()%>"
			class=buttons type=radio value="Approved"></td>
		<td align="center"><input name="status_<%=certDO.getCert_id()%>"
			class=buttons type=radio value="Rejected"></td>
		<%
			} else {
		%>
