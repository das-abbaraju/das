...
<form action="evaluation_view.jsp" method="post">
                <tr align="center">
                  <td align="left">
					...
					<span class="redMain"><strong>* Red questions are required and must be answered to be able to submit the evaluation</strong></span>
...
for(ContractorEvaluationCategory category: evalBean.getCategories()) {
	...
	for(ContractorEvaluationQuestion question: category.getQuestions() ) {
		...
		if (question.isRequired())
			...
			className = "blueMain";
		%>
		<tr bgcolor="<%=evalBean.getColor()%>" class="<%=className %>">
			<td valign="top" class="<%=className %>"><%=category.getDisplayOrder() %>.<%=question.getId()%></td>
			<td valign="top" class="<%=className %>"><%=question.getQuestion()%></td>
			<td width=50 valign="bottom"><%=question.printAnswers()%></td>
		</tr>
		<%
	}
}
%>
                ...
                <input type="hidden" name="requiredCount" value="29">
                <input type="hidden" name="auditType" value="PQF">
              </form>
