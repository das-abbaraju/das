...
public class ContractorLocationDAO extends PicsDAO{
    ...
    public List<ContractorLocation> findByViewPort(double neLat, double neLong, double swLat, double swLong, boolean safetySensitive) {
        Query query = em.createQuery("SELECT cl FROM ContractorLocation cl " +
                " JOIN cl.contractor ca " +
                " WHERE " +
                " cl.latitude > :swLat AND " +
                ...
                " cl.latitude < :neLat AND " +
                " cl.longitude < :neLong AND " +
                " ca.safetyRisk = :safetySensitive AND " +
                " (ca.status = :active OR ca.status = :pending ) ");
        query.setParameter("swLat", swLat);
        query.setParameter("swLong", swLong);
        query.setParameter("neLat", neLat);
    ...
    public List<ContractorLocation> findByViewPortAndTradeAndSafety(Trade trade, double neLat, double neLong, double swLat, double swLong, boolean safetySensitive) {
        Query query = em.createQuery("SELECT cl FROM ContractorLocation cl" +
                " JOIN cl.contractor ca "+
                " JOIN ca.trades ct " +
                " WHERE " +
