...
public class RequestedConWidget extends PicsActionSupport {
	...
	public List<BasicDynaBean> getRequestedContractors() throws Exception {
		...
		if (featureToggle.isFeatureEnabled(FeatureToggle.TOGGLE_REQUESTNEWCONTRACTORACCOUNT)) {
			...
			current.addWhere("(a.status = 'Requested' AND c.followupDate IS NULL) OR (a.status = 'Pending' AND c.requestedByID IS NOT NULL) OR (a.status = 'Declined' AND c.requestedByID IS NOT NULL and date_add(co.deadline, INTERVAL 30 DAY) > NOW()) ");
