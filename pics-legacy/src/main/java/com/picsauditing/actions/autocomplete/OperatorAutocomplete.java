...
@Deprecated
// This should be getting phased out with the release of Dynamic Report filters
	...
	protected Collection<OperatorAccount> getItems() {
		...
		if (itemKeys == null) {
			if (!Strings.isEmpty(q)) {
				if (isSearchDigit()) {
					return dao.findWhere(showCorporates, "a.id LIKE '%" + Strings.escapeQuotesAndSlashes(q) + "%'" + corpPerms,
							permissions);
				} else {
					return dao.findWhere(showCorporates, "a.name LIKE '%" + Strings.escapeQuotesAndSlashes(q) + "%'" + corpPerms,
							permissions);
				}
			}
