...
public class FusionChart {
	 ...
	 * @param strURL
	 *            - If you intend to use dataURL method for this chart, pass the URL as this parameter. Else, set it to
	 *            "" (in case of dataXML method)
	 * @param strXML
	 *            - If you intend to use dataXML method for this chart, pass the XML data as this parameter. Else, set
	 *            it to "" (in case of dataURL method)
	 * @param chartId
	 *            - Id for the chart, using which it will be recognized in the HTML page. Each chart on the page needs
	 *            to have a unique Id.
	 * @param chartWidth
	 ...
	 */
	public static String createChart(String chartSWF, String strURL, String strXML, String chartId, int chartWidth,
			int chartHeight, boolean debugMode, boolean registerWithJS) {
		...
		/*
		 * The above text "Chart" is shown to users before the chart has started loading (if there is a lag in relaying
		 * SWF from server). This text is also shown to users who do not have Flash Player installed. You can configure
		...
		/*
		 * Now, we create the chart using FusionCharts js class. Each chart's instance (JavaScript) Id is named as
		 * chart_"chartId".
		 */
		...
		strBuf.append("\t\t\t\tvar chart_" + chartId + " = new FusionCharts('" + chartSWF + "', '" + chartId + "', '"
		...
		if (strXML.equals("")) {
			...
			// "When using the HTML embedding method with data provided as embedded string, it is essential
			// to encode & character to %26."
			// 
			// www.fusioncharts.com/docs/SpPunctuation.html
			strXML = strXML.replaceAll("&", "%26");
	...
	/**
	 * Creates the object tag required to embed a chart. Generates the object tag to embed the swf directly into the
	 * html page.<br>
	 * Note: Only one of the parameters strURL or strXML has to be not null for this<br>
	 * method to work. If both the parameters are provided then strURL is used for further processing.<br>
	 * 
	 ...
	 *            - Whether to start the chart in debug mode
	 */
	public static String createChartHTML(String chartSWF, String strURL, String strXML, String chartId, int chartWidth,
			int chartHeight, boolean debugMode) { /*
