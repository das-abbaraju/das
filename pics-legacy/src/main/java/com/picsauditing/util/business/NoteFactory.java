...
/**
 * This class utilizes static factory methods to build notes anywhere in the
 * application where notes are generated from entity/model objects.
 * 
...
public class NoteFactory {
	...
	private static Note buildNoteForContractorTagging(ContractorTag contractorTag, Permissions permissions,
	...
	private static String buildNoteMessageForContractorTagging(ContractorTag contractorTag, Permissions permissions,
			String translationKey) {
		OperatorTag operatorTag = lookupOperatorTag(contractorTag.getTag().getId());
		return messageTagPrefix(operatorTag)
				+ TranslationServiceFactory.getTranslationService().getText(translationKey, Locale.US,
						permissions.getName(), permissions.getAccountName(), contractorTag.getUpdateDate());
	}
