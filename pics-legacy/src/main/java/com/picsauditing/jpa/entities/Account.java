...
public class Account extends AbstractIndexableTable implements Comparable<Account>, JSONable {
    ...
    @ReportField(i18nKeyPrefix = "Country", type = FieldType.Country, width = 150,
            importance = FieldImportance.Average)
    ...
    @ReportField(i18nKeyPrefix = "CountrySubdivision", type = FieldType.CountrySubdivision,
            importance = FieldImportance.Average)
     ...
     *
     * @return NOTE: "fetch=FetchType.LAZY" is a workaround for a EntityNotFoundException issue with no FK in the db. (
    ...
    @Type(type = "com.picsauditing.jpa.entities.EnumMapperWithEmptyStrings", parameters = {
            @Parameter(name = "enumClass", value = "com.picsauditing.jpa.entities.AccountStatus")})
    @Enumerated(EnumType.STRING)
    @Column(name = "status", nullable = false)
    @ReportField(type = FieldType.AccountStatus, i18nKeyPrefix = "AccountStatus",
    ...
    public void setStatus(AccountStatus status) {
        ...
        if (this.status == AccountStatus.Active && status == AccountStatus.Requested) {
            try {
                throw new Exception("Account " + id + " set from Active to Requested at "
                        + (new Date().toString()));
            } catch (Exception e) {
    ...
    public JSONObject toJSON(boolean full) {
        ...
        if (full) {
            ...
            obj.put("city", city);
            obj.put("countrySubdivision",
                    countrySubdivision == null ? null : countrySubdivision.getIsoCode());
    ...
    public String getSearchText() {
        ...
        if (this.country != null && this.country.isHasCountrySubdivisions()
    ...
    public boolean isMaterialSupplierOnly() {
        return (getAccountTypes().size() == 1 && getAccountTypes().iterator()
                .next()
                .equals(ContractorType.Supplier));
    }
    ...
    public EmailSubscription getFallbackSubscriptionForDefaultContact(Subscription subscription)
        ...
        User activeUserWithPermission = getActiveUserWithPermission(
                requiredPermissionForSubscription);
        if (activeUserWithPermission != null) {
            return activeUserWithPermission.getFallbackEmailSubscription(subscription);
        }

        User activeUserWithContractorAdminPermission = getActiveUserWithPermission(
                OpPerms.ContractorAdmin);
        if (activeUserWithContractorAdminPermission != null) {
            return activeUserWithContractorAdminPermission.getFallbackEmailSubscription(
                    subscription);
        }
    ...
    @Transient
    protected void addNewCurrentAccountUserOfRole(User newAccountUser, UserAccountRole role,
            int createdById) {
