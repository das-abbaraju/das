DROP PROCEDURE IF EXISTS	`ispRoleType`
;

DELIMITER //
CREATE PROCEDURE	ispRoleType
(
...
**	Purpose:	To insert RoleType data into tblRoleType
...
*/
###############################################################################
DECLARE	SYSTABLE	VARCHAR(255) DEFAULT 'tblRoleType';
DECLARE	SYSRIGHT	VARCHAR(40) DEFAULT 'INSERT';
DECLARE	Proc_nm	VARCHAR(255) DEFAULT 'ispRoleType';
...
BEGIN
	...
	THEN
		...
		(
			@Resrc_tp	:= Role_tp
		,	@ParentResrc_tp	:= ParentRole_tp
		,	@ResrcType_tx	:= RoleType_tx
		,	@Left_id	:= RoleTypeLeft_id
		,	@Right_id	:= RoleTypeRight_id
		,	@Level_id	:= RoleTypeLevel_id
		,	@Order_id	:= RoleTypeOrder_id
