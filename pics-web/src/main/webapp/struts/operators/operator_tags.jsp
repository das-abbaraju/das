		...
		<table class="table">
			<thead>
				<tr>
					...
					<th class="tag-category">
						<s:text name="OperatorTag.category"/>
					</th>
				...
				<tr class="<s:if test="#rowstatus.odd == true">odd</s:if><s:else>even</s:else>">
					...
					<s:if test="((permissions.operatorCorporate && permissions.hasPermission('ContractorTags', 'Edit') && operator.id == permissions.accountId)
						...
						<td class="tag-category">
							<s:select
								list="@com.picsauditing.jpa.entities.OperatorTagCategory@values()"
								listValue="%{getText(i18nKey)}"
								name="tags[%{#rowstatus.index}].category"
								value="%{category}"
							/>
						</td>
					...
					<s:else>
						...
						<td class="tag-category">
							<s:text name="%{#tag.category.i18nKey}" />
						</td>
			...
			<s:if test="permissions.hasPermission('ContractorTags','Delete') || permissions.marketing">
				<tfoot>
					<tr>
						<td colspan="7" class="add-new-tag">
