...
<tw:form formName="operator_project_companies_edit" action="${operator_project_companies_edit_url}" method="post" class="form-horizontal" autocomplete="off" role="form">
        ...
        <div class="col-md-4">
            ...
            <tw:select selectName="companies" multiple="true" class="form-control select2 companies-requested" tabindex="1">
                <s:iterator value="projectSites" var="project_company">
                    ...
                    <tw:option id="${project_company.id}" value="${project_company.id}" selected="${is_selected}">${project_company.name}</tw:option>
                </s:iterator>
            </tw:select>
        </div>
    </div>
