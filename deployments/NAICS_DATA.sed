...
# at http://www.bls.gov/iif/oshsum.htm into SQL statements. You are looking for
# a TXT file called "Summary Table #1: Incidence rates - detailed industry level".
#
...
#
#       sed -r -f NAICS_DATA.sed 2012NAICSStats.txt > 2012NAICSStats.sql
#
...
# records need to be converted into individual records, one for each code in the range, before this script is applied.



# Step 1. The s/.../.../ command below converts all data lines to SQL Insert statements.
# NOTE: Prior to the 2012 data, there used to be an extra column between the NAICS code and the TRIR total.
# So, if you need to re-covert an old file, you must pull an old copy of this script from version control, or it won't work.
#
#  [^|]*                                                                                  Industry
#             ([0-9]+)                                                                 \1 NAICS Code
...
#                                                   ([^ ]+)                            \4 LWCR
#                                                                ([^ ]+)               \5 restriction_transfer (not used in SQL statement)
#                                                                             ([^ ]+)  \6 other (not used in SQL statement)
s/^[^|]*\| +([0-9]+) +\| +([^ ]+) +\| +([^ ]+) +\| +([^ ]+) +\| +([^ ]+) +\| +([^ ]+).*$/INSERT INTO `naics` (`code`,`trir`,`dart`,`lwcr`) VALUES (\1,\2,\3,\4);/gi

# Step 2.  This s command changes "-" values to 0.0
s/,-/,0.0/gi

# Step 2b.  This s command changes "(10)" and "(11)" values to 0.0 ("(11)" is a footnote that means "too small to display" in pre-2012 reports; (10) in 2012 report)
s/,\(1[01]\)/,0.0/gi
