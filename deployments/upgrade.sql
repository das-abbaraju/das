...
update contractor_audit as ca
  join (select
          ca.id,
          max(caow.creationDate)    uDate
        from contractor_audit ca
          join contractor_audit_operator cao
            on cao.auditID = ca.id
          join contractor_audit_operator_workflow caow
            on caow.caoID = cao.id
        where caow.status = 'Submitted'
