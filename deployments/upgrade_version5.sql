...
-- adding categories/questions for WCB policies
insert into audit_category
select null,id,null,auditname,1,6,6,null,
1098,1098,Now(),Now(),0,null 
from audit_type 
...
or auditname like 'Newfoundland and Labrador%') and id not in (145);

select * from audit_type where (auditname like 'Alberta%'
or auditname like 'Ontario%'
or auditname like 'British Columbia%'
or auditname like 'Nova Scotia%'
or auditname like 'Manitoba%'
...
insert into audit_question 
select null,acnew.id,aq.number,aq.name,1098,1098,Now(),Now(),
aq.effectivedate,aq.expirationdate,aq.questiontype, 
aq.hasRequirement,aq.okAnswer,aq.required, 
aq.requiredQuestion,aq.requiredAnswer,aq.visibleQuestion,aq.visibleAnswer, 
aq.columnHeader,aq.uniqueCode,aq.title,aq.groupedWithPrevious,aq.flaggable, 
aq.showComment,aq.riskLevel,aq.helpPage,aq.requirement,aq.helpText
from audit_question aq
join audit_category ac on aq.categoryid = ac.id
join audit_category acnew on acnew.auditTypeID 
in 
(
select id from audit_type where (auditname like 'Alberta%'
or auditname like 'Ontario%'
or auditname like 'British Columbia%'
or auditname like 'Nova Scotia%'
or auditname like 'Manitoba%'
...
or auditname like 'Newfoundland and Labrador%')
and id not in (145))
where ac.auditTypeID = 145;

insert into audit_question 
select null,1157,aq.number,aq.name,1098,1098,Now(),Now(),
aq.effectivedate,aq.expirationdate,aq.questiontype, 
aq.hasRequirement,aq.okAnswer,aq.required, 
aq.requiredQuestion,aq.requiredAnswer,aq.visibleQuestion,aq.visibleAnswer, 
aq.columnHeader,aq.uniqueCode,aq.title,aq.groupedWithPrevious,aq.flaggable, 
aq.showComment,aq.riskLevel,aq.helpPage,aq.requirement,aq.helpText
from audit_question aq where aq.categoryid = 1158;
